...
def cl_to_unigene(cl, db):
    try:
        u = db.clusters.find({'cl' : cl})[0]['u']
    except:
        u = cl
    return u

def unigene_to_cl(u, db):
    cl = db.clusters.find({'u' : u})[0]['cl']
    return cl
...
def get_unigene_info(unigene, db, coll_clusters, coll_e5, ncbi):
    info = {
        ...
        'domains' : get_domains(unigene, db)
    }
...
def retrieve_newick(query_cluster, results_dir, client):
    treedb = client.trees
    fs = gridfs.GridFS(treedb)
    tfile = fs.find_one({"filename":query_cluster})
    treedata = fs.get(tfile._id)
    ...
    with open(results_dir + query_cluster + "_newick.txt", "w") as outputfile:
        outputfile.write(newick)
    print(len(cluster_unigenes))

def get_members(cl, client):
    members = client.gmgc_clusters.members.find({'cl' : cl})
    m = members[0]['clm']
    return m
...
def launch_analysis(query, n_range, cutoff, cluster=True, isList=False):
    ...
    if cluster:
        ...
        # unigene_list = get_unigenes_and_tree(query,
                                             ...
                                             # client)
        try:
            retrieve_newick(query,
                            RESULTS_PATH,
                            client)
        except:
            pass
        member_list = get_members(unigene_to_cl(query, db),
                                  client)
        unigene_list = [cl_to_unigene(m, db) for m in member_list]
        ...
        analysis = neighbor_analysis(query, unique_contigs=True, json=True)
